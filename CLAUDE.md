...
|-------|------|--------|------------------|-------------|----------------|
| `display-xl` | Hero wordmark ONLY | 54px | 128px | 0.95 / 0.92 | -0.02em |
| `display-lg` | Section titles, host name, dispatch headline | 39px | 70px | 1.06 / 1.02 | -0.015em |
| `display-md` | Featured ep title, pitch card title, principle numbers, colophon wordmark | 30px | 50px | 1.15 / 1.1 | -0.01em |
| `display-sm` | Upcoming episode titles, principle names, platform names | 25px | 30px | 1.22 | -0.005em |
| `body-lg` | Hero promise, premise, ledes, guest lede | 21px | 26px | 1.52 / 1.48 | 0 |
| `body-md` | Host bio, dispatch paragraph, pitch body, footer links | 17px | 18px | 1.6 / 1.6 | 0 |
| `body-sm` | Principle glosses (mono), featured ep desc, platform URL | 15px | 16px | 1.65 / 1.6 | 0 |
...
**Reduced 2026-08-05, then settled halfway.** The whole scale came down: body copy was far too large, and the rest followed so the proportions hold. The specific defect was `body-lg` at 32px against `display-sm` at 34px, so a paragraph was very nearly the size of an episode title and the hierarchy between them collapsed. The first cut (112/60/44/27 and `body-lg` 21) read too small, so the scale now sits at the midpoint of the original and that cut. `body-lg` still takes the deepest cut, 32 to 26 desktop, and is now clearly below `display-sm` at 30. Every measure on the page is in `ch`, so characters per line survived the change unchanged (hero 65, premise 72, episode lede 74, host bio 53 at 1440, unchanged across both moves). **If you change a size here, re-check the mobile Listen rows** (they are sized from `--body-md` and must clear 44px) and re-measure characters per line rather than assuming.
...
5. **Mobile cap:** `display-xl` at 54px mobile fits "Civilization." on all iPhones; on the 320px SE it is snug but fits.
...
- The size is `--body-md` (18px down to 768, 17px below). Rows are 47 to 48px, and the 12px vertical padding at ≤600 is the floor that keeps them over 44px, not a preference. **Do not reduce the desktop font size to prevent its wrap** — that is what produced the rejected band.
