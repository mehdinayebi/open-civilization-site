...
## Type, tone and rhythm system

### Semantic copy tokens

Running copy is sized by editorial role, not by a shared scale. Display tokens still drive titles.

| Token | Mobile | ≥768px | Used by |
|---|---|---|---|
| `--copy-hero` | 23px | 30px | `.hero-promise` |
| `--copy-thesis` | 22px | 27px | `.premise` |
| `--copy-lede` | 19px | 21px | `.section-lede`, `.guest-lede` |
| `--copy-body` | 16px | 18px | `.host-bio p`, `.dispatch p`, `.pitch-card-body` |
| `--copy-ledger` | 14px | 15px | `.tx-desc`, `.tx-question`, `.doctrine-gloss` |

**Define every token in the base `:root`, not only in the 768px override.** A patch once defined them only in the override, which left every `var()` undefined on mobile and silently killed the rules that used them, including all the `--rule-*` borders. A test now asserts base-`:root` definition and that every `var()` used anywhere resolves.

### The two ch traps

`ch` is the width of the "0" glyph, and it resolves against **the element's own font**.

1. **Serif measures are not character counts.** Fraunces' figures are much wider than its average letter, so a `ch` value buys roughly 35% more characters than the number suggests. Never set a serif measure by assuming `ch` equals a character.
2. **Never put a measure on a wrapper.** A plain `div` inherits mono 16px, so `ch` there is computed from the wrong font entirely. `.premise-wrap { max-width: 37ch }` rendered 28 characters a line, not 62. **Measures belong on the text element.**

Mono is the exception: for IBM Plex Mono, `ch` is exactly one character, so `.host-bio p { max-width: 57ch }` means 57 characters.

Measured targets, verified by counting rendered characters at 1440px:

| Element | Target | Value | Actual |
|---|---|---|---|
| `.hero-promise` | 58 to 66 | 47ch | 66 |
| `.premise` | 60 to 64 | 45ch | 63 |
| `.section-lede--episodes` | 58 to 62 | 43ch | 61 |
| `.section-lede--principles` | 56 to 60 | 42ch | 57 |
| `.host-bio p` | 55 to 60 | 57ch (mono, exact) | 55 |
| `.guest-lede` | 30 to 36 | 24ch | 33 |
| `.dispatch p` | 48 to 54 | 54ch | 53 |

Retune by measuring, never by arithmetic on the ch number.

### Tonal sequence

Explicit per section, never `nth-child`. `nth-child` is for animation only; a test fails if it ever touches a background.

> hero paper / premise tint / episodes paper / principles tint / host paper / guests tint / listen paper / dispatch ink / footer tint

Classes: `.section--paper`, `.section--tint`, `.section--ink`. The old implicit `.tinted` class is gone. Hover surfaces move away from whichever ground they sit on, so `.section--tint` hovers go lighter.

### Section archetypes

- **Statement** (hero, premise): large serif, controlled measure, no rules inside.
- **Ledger** (episodes): number, title, summary, question. Full width.
- **Doctrine** (principles): two-column editorial index at ≥1100px, 01 to 05 left and 06 to 10 right, one divider rule. Document order stays 01 to 10 so screen readers and the visual columns agree. Single sequence below 1100px.
- **People** (host, guests): host is narrow measure on paper with air to the right; guests is the asymmetric lede plus bordered card on tint.
- **Utility** (listen): compact band, roughly 180 to 220px tall, not a full chapter.
- **Conversion** (dispatch): the only dark section.

### Rule hierarchy

`--rule-major-width` 1.5px for section boundaries, `--rule-section-width` 1px for section heads and the principles divider, `--rule-row-width` 1px for ledger rows. **No horizontal rule ever separates paragraphs inside a section.** At `dpr: 1` browsers snap 1.5px to 1px; the hierarchy shows on retina.

### Active masthead state

A third IntersectionObserver, isolated from the reveal and doctrine observers. It only decides *when* to recompute; the answer is re-derived from geometry each time, because accumulating `intersectionRatio` goes stale once a tall section fills the band. A reading line at 38% of viewport height picks the section containing it.

`aria-current="location"` gets red plus an underline, never colour alone. Hero is not observed, so nothing is active there. Guests is observed but has no nav link, so it clears every link, which is intended.

**The preview environment's IntersectionObserver does not fire**, so this cannot be verified there. Verify the geometry predicate directly, or test in a real browser.

### Section rhythm

Vertical padding varies by role; horizontal padding is constant. Desktop: hero 120/82, premise 72/78, episodes, principles and guests 88/90, host 82/86, listen 50/52, dispatch 90/92, footer 64/34. Tablet and mobile scale down in the `1000px` and `600px` blocks.
